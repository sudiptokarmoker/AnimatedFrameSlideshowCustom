    ...
    class Slideshow {
        ...
        initEvents() {
           ...
           if(this.slidesTotal > 0){
            ...
            let currentIndexPostion = 0;
            items.forEach(function(item, key){
                //item.addEventListener('click', () => console.log(currentIndexITem));
                item.addEventListener('click', function(){
                    //box1.classList.contains('active');
                    /**
                     * find where the item is avalible or not
                     */
                    document.querySelectorAll('.slides .slide').forEach(function(element, key){
                        let isCurrentElement = element.classList.contains('slide--current');
                        if(isCurrentElement === true){
                            currentIndexPostion = key;
                        }
                    });
                    //console.log("currentIndexPostion : " + currentIndexPostion);
                    //console.log("key : " + key);
                    console.log(self);
                    //if(key > currentIndexPostion){
                        //navigate('next');
